<footer class="text-center text-lg-start bg-white text-muted">
    <section class="" style="background-color: rgba(0, 0, 0, 0.025);">
        <div class="container text-center text-md-start mt-5">
            <div class="row mt-5">
                <div class="col-md-3 col-lg-4 col-xl-3 mx-auto mb-4 mt-3">
                    <h6 class="text-uppercase fw-bold mb-4" style="font-family:MV BOLI; font-size:200%;">
                        <a href="<@spring.url '/'/>" style="text-decoration: none; color: inherit">
                            Religious studying
                        </a>
                    </h6>
                    <p>
                        Religious studying is a site where you can start your road in studying different
                        religions or to help others with their study... <a class="text-reset" style="color: #0b5ed7"
                                                                           href="<@spring.url '/about-us'/>">Read more</a>
                    </p>
                </div>

                <div class="col-md-3 col-lg-2 col-xl-2 mx-auto mb-4 mt-3">
                    <h6 class="text-uppercase fw-bold mb-4">
                        Help
                    </h6>
                    <p>
                        <a href="<@spring.url '/about-us'/>" class="text-reset">About us</a>
                    </p>
                    <p>
                        <a href="<@spring.url '/terms-conditions'/>" class="text-reset">Terms & Conditions</a>
                    </p>
                    <p>
                        <a href="<@spring.url '//privacy-policy'/>" class="text-reset">Privacy policy</a>
                    </p>
                </div>

                <div class="col-md-4 col-lg-3 col-xl-3 mx-auto mb-md-0 mb-4 mt-3">
                    <h6 class="text-uppercase fw-bold mb-4">Contact</h6>
                    <p>
                        religious-studying@gmail.com
                    </p>
                    <p>+7 982 633 59 64</p>
                </div>
            </div>
        </div>
    </section>

    <div class="text-center p-4" style="background-color: rgba(0, 0, 0, 0.05);">
        © 2023 Copyright:
        <a class="text-reset fw-bold" href="<@spring.url '/'/>">Religious studying</a>
        All rights reserved
    </div>
</footer>
